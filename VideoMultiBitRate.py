...
# the output video container is mp4,with gop:15, total frames:1800, frame rate:30, codec:h.265
...
def main():
	...
	for x in range(1, TileNum_w+1):
		for y in range(1, TileNum_h+1):
			for bitv in BitNum:
				os.system("ffmpeg -f rawvideo -s {0} -r 30 -i {1} -b:v {2} -q 1 -r 30 -vframes 1800 -g 15 -c:v libx265 {3}".format \
				( tile_size, \
				  "w"+str(x)+"h"+str(y)+".yuv", \
			       bitv, \
