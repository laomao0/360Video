...
for w = 0 : tileNum_Width - 1
    ...
    for h = 0 : tileNum_Height - 1
		y = h * tile_Height;
        cmd = sprintf('D:/ffmpeg/bin/ffmpeg -s %s -i D:/ffmpeg/bin/abc.yuv -filter:v "crop=%d:%d:%d:%d" D:/ffmpeg/bin/abc_w%02d-h%02d.yuv', Frame_size, tile_Width, tile_Height, x, y, w+1, h+1);%original video resolution, tiled vidoe resoluton,  x y represent relative position
        cmd
        unix(cmd);
	end
end
