...
def main():
    ...
    for log_file in log_files:
        ...
        with open(RESULTS_FOLDER + log_file, 'rb') as f:
            if SIM_DP in log_file:
                ...
                for line in f:
                    ...
                    # [0](str(time_stamp / M_IN_K) + '\t' +  # unit: sec
                    # [1]str(video_chunk_quality) + '\t' +  # unit: Kbps
                    # [2]str(buffer_size) + '\t' +  # unit: sec
                    # [3]str(rebuf) + '\t' +  # unit: sec
                    # [4]str(video_chunk_size) + '\t' +  # unit: Bytes
                    # [5]str(delay) + '\t' +  # unit: ms
                    # [6]str(reward) + '\n')
        ...
        for scheme in SCHEMES:
            if scheme in log_file:  # check sim_method is or is not in log file
    ...
    for l in time_all[SCHEMES[0]]:  # find the same through put trace file of different buffer method for comparison
        schemes_check = True
        for scheme in SCHEMES:
